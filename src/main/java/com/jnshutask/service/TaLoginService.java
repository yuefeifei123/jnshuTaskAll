...
public interface TaLoginService {
     ...
     * @return 成功为1；失败为-999
     */
    boolean logining(TaLogin login);
     ...
     * @return 存在为true；
     */
    boolean existAll(TaLogin login);
     ...
     */
    boolean verifyFormat(TaLogin login);
     ...
     * @return 查出的分页记录；
     */
    List<TaLogin> pageLogin(Integer pageNum, Integer pageSize);

    long countLogin();

    TaLogin selectById(Long loginId);
}
